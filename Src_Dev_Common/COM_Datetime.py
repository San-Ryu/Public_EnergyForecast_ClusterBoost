...
def list_invalidDate(df_tar, col_tar):
    print(df_tar.shape)
    list_errValues = [] 
    for i in range(0, len(df_tar)): validate_date(str(df_tar[col_tar].iloc[i]), i, list_errValues)
...
def create_col_ymdhm(df_tar, col_tar):
    ...
    df_tar['YEAR'], df_tar['MONTH'], df_tar['DAY'] = df_tar[col_tar].dt.year, df_tar[col_tar].dt.month, df_tar[col_tar].dt.day
    df_tar['HOUR'], df_tar['MINUTE'] = df_tar[col_tar].dt.hour, df_tar[col_tar].dt.minute
...
def create_col_weekdays(df_tar, tar_col):
    ...
    for i in range(0, len(df_tar)): df_tar['code_day_of_the_week'].iloc[i] = dict_weekday.get(df_tar['day_of_the_week'].iloc[i])
...
def create_col_datetime(df_tar, col_tar, col_y, col_m, col_d
    ...
    df_tar.insert(loc=0, column = col_tar, value = 0)
